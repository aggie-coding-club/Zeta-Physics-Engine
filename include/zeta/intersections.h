...
namespace Collisions {
    ...
    static bool PointAndLine(ZMath::Vec3D const &point, Primitives::Line3D const &line) {
    ...
    static bool PointAndPlane(ZMath::Vec3D const &point, Primitives::Plane const &plane) {
    ...
    static bool PointAndSphere(ZMath::Vec3D const &point, Primitives::Sphere const &sphere) { return sphere.c.distSq(point) <= sphere.r*sphere.r; };

    // Determine if a point lies within an unrotated cube.
    static bool PointAndAABB(ZMath::Vec3D const &point, Primitives::AABB const &aabb) {
    ...
    static bool PointAndCube(ZMath::Vec3D const &point, Primitives::Cube const &cube) {
    ...
    static bool LineAndPoint(Primitives::Line3D const &line, ZMath::Vec3D const &point) { return PointAndLine(point, line); };

    // Determine if a line intersects another line.
    static bool LineAndLine(Primitives::Line3D const &line1, Primitives::Line3D const &line2) {
    ...
    static bool LineAndPlane(Primitives::Line3D const &line, Primitives::Plane const &plane) {
    ...
    static bool LineAndSphere(Primitives::Line3D const &line, Primitives::Sphere const &sphere) {
    ...
    static bool LineAndAABB(Primitives::Line3D const &line, Primitives::AABB const &aabb) {
    ...
    // todo this might not work
    static bool LineAndCube(Primitives::Line3D const &line, Primitives::Cube const &cube) {
    ...
    static bool raycast(Primitives::Plane const &plane, Primitives::Ray3D const &ray, float &dist) {
    ...
    // dist is set to -1 if there is no intersection.
    static bool raycast(Primitives::Sphere const &sphere, Primitives::Ray3D const &ray, float &dist) {
    ...
    // dist is set to -1 if there is no intersection.
    static bool raycast(Primitives::AABB const &aabb, Primitives::Ray3D const &ray, float &dist) {
    ...
    // dist is set to -1 if there is no intersection.
    static bool raycast(Primitives::Cube const &cube, Primitives::Ray3D const &ray, float &dist) {
    ...
    static bool PlaneAndPoint(Primitives::Plane const &plane, ZMath::Vec3D const &point) { return PointAndPlane(point, plane); };

    // Determine if a plane intersects a line.
    static bool PlaneAndLine(Primitives::Plane const &plane, Primitives::Line3D const &line) { return LineAndPlane(line, plane); };

    // Determine if a plane intersects a sphere.
    static bool PlaneAndSphere(Primitives::Plane const &plane, Primitives::Sphere const &sphere) {
    ...
    static bool PlaneAndAABB(Primitives::Plane const &plane, Primitives::AABB const &aabb) {
    ...
    static bool PlaneAndCube(Primitives::Plane const &plane, Primitives::Cube const &cube) {
    ...
    static bool SphereAndPoint(Primitives::Sphere const &sphere, ZMath::Vec3D const &point) { return PointAndSphere(point, sphere); };

    // Determine if a sphere intersects a line.
    static bool SphereAndLine(Primitives::Sphere const &sphere, Primitives::Line3D const &line) { return LineAndSphere(line, sphere); };

    // Determine if a sphere intersects a plane.
    static bool SphereAndPlane(Primitives::Sphere const &sphere, Primitives::Plane const &plane) { return PlaneAndSphere(plane, sphere); };

    // Determine if a sphere intersects another sphere.
    static bool SphereAndSphere(Primitives::Sphere const &sphere1, Primitives::Sphere const &sphere2) {
    ...
    static bool SphereAndSphere(Primitives::Sphere const &sphere1, Primitives::Sphere const &sphere2, ZMath::Vec3D &normal) {
        float r = sphere1.r + sphere2.r;
    ...
    static bool SphereAndAABB(Primitives::Sphere const &sphere, Primitives::AABB const &aabb) {
    ...
    // If there is not an intersection, the normal will be a junk value.
    // The normal will point towards B away from A.
    static bool SphereAndAABB(Primitives::Sphere const &sphere, Primitives::AABB const & aabb, ZMath::Vec3D &normal) {
        ZMath::Vec3D closest = sphere.c;
        ZMath::Vec3D min = aabb.getMin(), max = aabb.getMax();
    ...
    static bool SphereAndCube(Primitives::Sphere const &sphere, Primitives::Cube const &cube) {
    ...
    // If there is not an intersection, the normal will be a junk value.
    // The normal will point towards B away from A.
    static bool SphereAndCube(Primitives::Sphere const &sphere, Primitives::Cube const &cube, ZMath::Vec3D &normal) {
        ZMath::Vec3D closest = sphere.c - cube.pos;
        ZMath::Vec3D min = cube.getLocalMin(), max = cube.getLocalMax();
    ...
    static bool AABBAndPoint(Primitives::AABB const &aabb, ZMath::Vec3D const &point) { return PointAndAABB(point, aabb); };

    // Determine if an unrotated cube intersects a line.
    static bool AABBAndLine(Primitives::AABB const &aabb, Primitives::Line3D const &line) { return LineAndAABB(line, aabb); };

    // Determine if an unrotated cube intersects a plane.
    static bool AABBAndPlane(Primitives::AABB const &aabb, Primitives::Plane const &plane) { return PlaneAndAABB(plane, aabb); };

    // Determine if an unrotated cube intersects a sphere.
    static bool AABBAndSphere(Primitives::AABB const &aabb, Primitives::Sphere const &sphere) { return SphereAndAABB(sphere, aabb); };

    // Check for intersection and return the collision normal.
    // If there is not an intersection, the normal will be a junk value.
    // The normal will point towards B away from A.
    static bool AABBAndSphere(Primitives::AABB const &aabb, Primitives::Sphere const &sphere, ZMath::Vec3D &normal) {
    ...
    static bool AABBAndAABB(Primitives::AABB const &aabb1, Primitives::AABB const &aabb2) {
    ...
    // If there is not an intersection, the normal will be a junk value.
    // The normal will point towards B away from A.
    static bool AABBAndAABB(Primitives::AABB const &aabb1, Primitives::AABB const &aabb2, ZMath::Vec3D &normal) {
    ...
    static bool AABBAndCube(Primitives::AABB const &aabb, Primitives::Cube const &cube) {
    ...
    // If there is not an intersection, the normal will be a junk value.
    // The normal will point towards B away from A.
    static bool AABBAndCube(Primitives::AABB const &aabb, Primitives::Cube const &cube, ZMath::Vec3D &normal) {
        // ? Use the separating axis theorem to determine if there is an intersection between the AABB and cube.
    ...
    static bool CubeAndPoint(Primitives::Cube const &cube, ZMath::Vec3D const &point) { return PointAndCube(point, cube); };

    // Determine if a cube intersects a line.
    static bool CubeAndLine(Primitives::Cube const &cube, Primitives::Line3D const &line) { return LineAndCube(line, cube); };

    // Determine if a cube intersects a plane.
    static bool CubeAndPlane(Primitives::Cube const &cube, Primitives::Plane const &plane) { return PlaneAndCube(plane, cube); };

    // Determine if a cube intersects a sphere.
    static bool CubeAndSphere(Primitives::Cube const &cube, Primitives::Sphere const &sphere) { return SphereAndCube(sphere, cube); };

    // Check for intersection and return the collision normal.
    // If there is not an intersection, the normal will be a junk value.
    // The normal will point towards B away from A.
    static bool CubeAndSphere(Primitives::Cube const &cube, Primitives::Sphere const &sphere, ZMath::Vec3D &normal) {
    ...
    static bool CubeAndAABB(Primitives::Cube const &cube, Primitives::AABB const &aabb) { return AABBAndCube(aabb, cube); };

    // Check for intersection and return the collision normal.
    // If there is not an intersection, the normal will be a junk value.
    // The normal will point towards B away from A.
    static bool CubeAndAABB(Primitives::Cube const &cube, Primitives::AABB const &aabb, ZMath::Vec3D &normal) {
    ...
    static bool CubeAndCube(Primitives::Cube const &cube1, Primitives::Cube const &cube2) {
    ...
    // If there is not an intersection, the normal will be a junk value.
    // The normal will point towards B away from A.
    static bool CubeAndCube(Primitives::Cube const &cube1, Primitives::Cube const &cube2, ZMath::Vec3D &normal) {
        // ? Use the separating axis theorem to determine if there is an intersection between the cubes.
