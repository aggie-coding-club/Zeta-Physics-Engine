...
bool testLineAndCube() {
    ...
    line.end.set(-0.707107f, 1.4142f, 0.707107f);

    if (UNIT_TEST("Line on edge of Cube.", Collisions::LineAndCube(line, cube3), 1)) { return 1; }
...
bool testRaycastingVSAABB() {
    ...
    if (RAYCAST_TEST("Ray and AABB.", Collisions::raycast(aabb1, ray, dist), 1, 1.73205f, dist)) { return 1; }

    Primitives::AABB aabb2(ZMath::Vec3D(-2), ZMath::Vec3D(2));

    if (RAYCAST_TEST("Ray starting in AABB.", Collisions::raycast(aabb2, ray, dist), 1, 1.73205f, dist)) { return 1; }
...
bool testRaycastingVSCube() {
    ...
    // ! will probably wait a bit to do this since it'll take a good bit of time
